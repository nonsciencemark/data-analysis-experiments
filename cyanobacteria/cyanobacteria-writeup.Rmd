...
```{r, plotting-raw-data, fig.width=10, fig.height=8, fig.cap="Strain population change over time during 7 days of growth"}
# Load data
in.data <- readRDS('C:/Users/mholmes/Documemts UNamur/Lab work/03_Experiments/15_2022-06_DIVERCE/mono_data_clean.RData') %>% #readRDS('mono_data_clean.RData') %>%
...
# View raw data
ggplot(in.data %>% dplyr::filter(date > '2022-06-17'), 
       aes(x = date, y = population, col = strain)) + 
  ...
  geom_pointrange(aes(ymin = population - population.se, 
                      ymax = population + population.se), 
...
# model fitting
mod <- in.data %>%
  summarise(gause.fit = list(gause_wrapper( # gauseR function
    time = date.time, # sampling intervals
    species = population, # population densities
    keeptimes = TRUE, # return prediction at the same time-steps
    r_starting = 5e-2, # initial guess for r
    A_starting = -5e-7, # initial guess for a 
...
out.data <- pred.data %>% 
  full_join(in.data) %>%
  mutate(d.pred = abs(prediction - population))
...
ggplot(out.data, aes(x = date.time, col = strain)) + 
  ...
  geom_point(aes(y = population), pch = 21, fill = 'white', size = 2) +
  theme_bw() +
  labs(col = 'Strain', x = 'Time (hours)', y = 'Population Density') +
  theme(legend.position = 'bottom')
```
...
```{r, coefficient-of-determination, fig.width=5, fig.height=3, fig.cap="Accuracy of model-fitting procedure"}
# Data frame with CoD
CoD.data <- out.data %>%
  mutate(difference = population - prediction,
         diff.squared = difference^2) %>%
  group_by(strain, treat) %>%
  summarise(RSS = sum(diff.squared),
            TSS = sum((population - mean(population))^2),
            R2 = 1 - (RSS / TSS),
            pop.range = population %>% range %>% diff,
...
We can see that the model fit is mostly good except for strain 2383 in atrazine conditions.

```{r}
in.data %>% dplyr::filter(date > '17-06-2022') %>%
  mutate(lag.pop = get_lag(population, date.time),
                   PCGR = percap_growth(lag.pop$x, lag.pop$laggedx, lag.pop$dt)) %>%
  ggplot(aes(x = lag(population), y = PCGR, col = strain)) + 
  facet_wrap(strain~treat, scales = 'free', nrow = 4,
             labeller = label_wrap_gen(multi_line=FALSE)) +
  scale_x_log10() +
  geom_point(pch = 21, fill = 'white', size = 2) +
  theme_bw() +
...
trait.time <- out.data %>%
  ...
  mutate(across(FSC.HLin:RED.R.HLin, .fns = function(i) {i$mean})) %>%
  pivot_longer(FSC.HLin:RED.R.HLin, names_to = 'Channel')

# plot
ggplot(trait.time, aes(x = date.time, y = value, col = strain, pch = treat)) + 
...
trait.change <- trait.time %>%
  ...
  summarise(change = (first(value) - last(value)) / first(value))#sd(value) / mean(value) * 100)
...
ggplot(mod.df, aes(x = strain, y = value, col = strain, pch = treat)) +
  ...
  geom_path(lty = 2, position = position_dodge(.5)) +
  geom_point(size = 2, fill = 'white', 
...
# plot
ggplot(mod.traits, aes(x = Channel.value, y = Parameter.value, col = strain)) +
  facet_grid(Parameter~Channel, scales = 'free') +
  scale_y_continuous(trans = 'pseudo_log') +
  geom_path(lty = 2) +
  geom_point(aes(pch = treat), size = 2, fill = 'white') +
  scale_shape_manual(values = 21:24) +
  theme_bw() +
  theme(legend.position = 'bottom')
```
...
Let's try without that outlier

```{r}
# plot 
ggplot(dpar %>% dplyr::filter(!(strain == 2383 & Parameter == 'aii')), 
       aes(x = Channel.value, y = Parameter.change, col = strain)) +
  facet_grid(Parameter~Channel, scale = 'free') +
  geom_hline(yintercept = 1, lty = 2) +
  geom_point(size = 2, fill = 'white', pch = 21) +
  theme_bw() + 
