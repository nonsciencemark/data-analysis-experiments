...
AIC_plots <- function(model_system) {
...
third_plots <- function(model_system) {
...
td_general_plots <- function(model_system) {
  ...
  stats_result <- modelling(data = data, 
    ...
    mutate(type_of_pred = ifelse(
      length(grep("density", predictor) > 0) | length(grep("trait", predictor) > 0), 
      "slope", "intercept")) %>%
    ungroup() %>%
