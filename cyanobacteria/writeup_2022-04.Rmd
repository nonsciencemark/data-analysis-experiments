...
```{r, plotting-raw-data, fig.width=5, fig.height=8, fig.cap="Strain population change over time during 7 days of growth"}
# Data location
path <- 'C:/Users/mholmes/Documents UNamur/Lab work/03_Experiments/12_2022-01-14_DIVERCE/'

# Load data
in.data <- readRDS(paste0(path, 'mono_data_clean.RData')) %>%
...
# View raw data
ggplot(in.data, aes(x = date.time, y = population, col = strain)) + 
...
```{r, fitting-parameters, fig.width=5, fig.height=8, fig.cap="Prediction of fitted Lotka-Volterra model"}
# model fitting
mod <- in.data %>%
...
# plot
ggplot(mod.df, aes(x = strain, y = value, col = strain)) +
