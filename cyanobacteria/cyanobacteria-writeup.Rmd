...
mono.data %>% dplyr::filter(date > '17-06-2022') %>%
  mutate(lag.pop = get_lag(population.mean, date.time),
                   PCGR = percap_growth(lag.pop$x, lag.pop$laggedx, lag.pop$dt)) %>%
  ggplot(aes(x = lag(population.mean), y = PCGR, col = treat)) + 
...
ggplot(mod.traits %>% dplyr::filter(parameter == 'aii'), 
       aes(x = channel.value, y = -parameter.value)) +
